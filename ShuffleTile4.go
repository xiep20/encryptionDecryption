...
func listAll(path string, curHier int) {
	...
	for _, info := range readerInfos {
		if info.IsDir() {
			...
			var namelist = strings.Split(info.Name(), ".")
			v3, verr3 := strconv.Atoi(namelist[0])
			if verr3 != nil {
			}
			...
			// }
			moveFile(path+"\\"+info.Name(), newpath+"\\"+a2+b2+c2+"."+namelist[1])
...
func moveFile(oldLocation, newLocation string) {
	...
	total, err := CopyFile2(oldLocation, newLocation) //复制文件

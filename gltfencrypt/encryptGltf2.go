...
func main() {
	fmt.Printf("请输入加密文件url：")
	fmt.Scan(&oldpath)
	fmt.Printf("请输入文件加密后保存url：")
	fmt.Scan(&newpath)
	fmt.Printf("请输入文件加密后解密保存url：")
	fmt.Scan(&newpath2)
	EncryptFile(oldpath, newpath) //加密
	DecryptFile(newpath, newpath2)
}

var PwdKey = []byte("ABCDEF1234567890")
...
func EncryptFile(fileName, fName string) (err error) {
	...
	// encryptd to file
	ff, err := os.OpenFile(fName, os.O_RDWR|os.O_CREATE, 0666)
	if err != nil {
		fmt.Println("文件写入错误")
		return err
	}
...
// DecryptFile 文件解密
func DecryptFile(fileName, fName string) (err error) {
	f, err := os.Open(fileName)
	if err != nil {
		fmt.Println("未找到文件")
		return
	...
	ff, err := os.OpenFile(fName, os.O_WRONLY|os.O_CREATE, 0666)
	if err != nil {
		fmt.Println("文件写入错误")
		return err
	}
